...
(function (AJAX,HUB,HTMLCSS) {
  ...
  MathJax.Hub.Register.StartupHook("mml Jax Ready",function () {
	...
	return span;
      ...
      // span is required, marker is optional string.  If given it is added to id of element created to make it unique.
      // Without this, calling this multiple times with the same span will result in elements with non unique ID
	...
	if (values.mathcolor || values.mathcolor === "") {span.style.color = values.mathcolor}
	...
	if ((values.mathbackground && values.mathbackground !== MML.COLOR.TRANSPARENT) || 
	  ...
	  if (W > 0) {W += 2*dd; lW -= dd}; if (H > 0) {H += 2*dd; D -= dd}; rW = -W-lW;
          ...
          if(typeof(marker) === "string") {
            ...
            if(document.getElementById(strSpanID) != null) {   // when marker given ensure that the ID is unique
              return null;
            }
          }
          else {
  ...
  HUB.Highlight = {
    elementHelper: (function () {
      function LoopState() {
        this.count = 2; // When loop starts already at the second element
        this.list = [];
      }
      ...
      function looper(dataElem, start, end, state) {
        if (!dataElem.inferred) {
          // if is multi character token is handled separately
          var bHandledToken = false;
        ...
        if (dataElem.type === "mspace") {   // don't go to children of this if any (based on toMathML skipping this
          return false;
        }
    ...
    getHighlighter: (function () {
      ...
      var Highlighter = function (jax) {
        ...
        this.setHighlightStyle = function (type, style, r, g, b) {
          ...
          catch (err) {   // invalid values passed in
            return false;
          }
